...
<script>
  ...
  app.controller('myCtrl', function($scope) {
    ...
    <div class="col-md-3">
      <h4 align="center"><img src="{{asset("icons/star1.png")}}"><b>เเนะนำสำหรับคุณ</b><img src="{{asset("icons/star1.png")}}"></h4>

       <?php
        if((int)Auth::user()->group == 1)
        {
          foreach ($baye_rating1 as $value) 
          {
            echo "<p style='color:red;'><img src='../icons/thumbs-up-animated.gif' width='37px' height='32px' >$value[0]</p>";
          }
        }
        else if((int)Auth::user()->group == 2)
        {
          foreach ($baye_rating2 as $value) 
          {
            echo "<p style='color:red;'><img src='../icons/thumbs-up-animated.gif' width='37px' height='32px' >$value[0]</p>";
          }
        }
        else if((int)Auth::user()->group == 3)
        {
          foreach ($baye_rating3 as $value) 
          {
            echo "<p  style='color:red;'><img src='../icons/thumbs-up-animated.gif' width='37px' height='32px' >$value[0]</p>";
          }
        }
        else if((int)Auth::user()->group == 0)
        {
          echo "<center><p style='color: red;'>ไม่มีรายการที่เเนะนำ</p></center><br>";
          echo "<center><a class='btn btn-primary' href='/member/fill_factors'>คลิกที่นี่เพื่อเข้าสู่ขั้นตอนการเเนะนำ</a></center>";
        }
      ?> 
      ...
      {!! Form::open(['action' => "MemberController@searchType"]) !!}
          ...
          ({
            ...
            infoWindow: 
            {
              ...
              content: '<span style="padding: 0px; text-align:left"  ><img src="../images/fitnesses/'
                       ...
                       +value.fitness_ename+ '&nbsp; [' +value.fitness_tname+ ']</h4><p align="center">'+value.fitness_description+'</p><hr><h5><b>ที่อยู่ :: </b>' +value.addresses+ '<br /></h5><h5><b>ติดต่อ :: </b>'+value.contact_number+ '</h5>'+'<h5><b>เว็บไซต์ :: </b><a target="_blank" href="' +value.website+  '">'+value.website+'</a></h5><h5><b>วัน-เวลาเปิดทำการ :: </b>'+work+'</h5><h5><b>ประเภทของสถานที่ออกกำลังกาย :: </b>'+type+'</h5><h5><b>คลาสที่เปิดสอน :: </b>'+classes+'</h5><center><a href="/member/route?fitness=' +value.fitness_tname+ '&&lat=' +value.lat+ '&&lng='+value.lng+'" class="btn btn-info" role="button">เเสดงเส้นทางการเดินทาง</a></center></span>'
            }
          });
      });
